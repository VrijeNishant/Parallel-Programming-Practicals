...
void do_asp(int **my_tab, int n, int lb, int ub, int **tab) {
	int i, j, k, tmp;
...
int main(int argc, char *argv[]) {
	...
	while(m_times_loop<(n-1)){
	
	start_time = MPI_Wtime();

	ret = MPI_Bcast(tab_buf, n*n, MPI_INT, 0, MPI_COMM_WORLD);
